...
#   - Fix cuSOLVER errors by computing matrix inverse on CPU
#   - Fix object_mask path to match our directory structure (images_all → object_mask_all)
...
class Camera(nn.Module):
    def __init__(self, resolution, colmap_id, R, T, Cx, Cy, FoVx, FoVy, image, alpha_mask,
        ...
        # FIX: compute inverse on CPU to avoid cuSOLVER errors in long-running pipelines
        self.camera_center = self.world_view_transform.cpu().inverse().cuda()[3, :3]

        #! default pinhole camera model, which needs to be updated
        ...
        # FIX: compute inverse on CPU
        self.c2w = self.world_view_transform.transpose(0, 1).cpu().inverse().cuda()

        # FIX: use object_mask_all to match our directory structure
        object_mask_path = image_path.replace("images_all", "object_mask_all").replace("images", "object_mask").replace(".jpg", ".png").replace(".JPG", ".png")
        try:
            object_mask = Image.open(object_mask_path).convert('L')
        except FileNotFoundError:
            # Fallback: create a white mask (whole image = one object)
...
class MiniCam:
    def __init__(
        ...
        # FIX: compute inverse on CPU
        view_inv = torch.inverse(self.world_view_transform.cpu()).cuda()
